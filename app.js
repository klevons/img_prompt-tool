...
const REFERENCE_IMAGES = [
    { id: 'ref-1', label: '中式烤鸭-园林', emoji: '🦆', color: '#D4A373' },
    { id: 'ref-2', label: '日式寿司-木盘', emoji: '🍣', color: '#CCD5AE' },
    { id: 'ref-3', label: '甜点-午后阳光', emoji: '🍰', color: '#F2D0C6' },
    { id: 'ref-4', label: '咖啡-天台夜景', emoji: '☕', color: '#A68A7A' },
...
function openInspirationModal() {
    ...
    grid.innerHTML = REFERENCE_IMAGES.map(img => {
        ...
        const gradient = `linear-gradient(135deg, ${img.color}, ${adjustColor(img.color, -30)})`;
        return `
            <div class="image-grid-item ${isSelected ? 'selected' : ''}" data-id="${img.id}" onclick="selectImage('${img.id}')">
                <div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:${gradient};font-size:48px;">
                    ${img.emoji}
                </div>
...
function confirmImageSelection() {
    ...
    const gradient = `linear-gradient(135deg, ${img.color}, ${adjustColor(img.color, -30)})`;
    preview.innerHTML = `
        <div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:${gradient};border-radius:4px;font-size:48px;min-height:70px;">
            ${img.emoji}
        </div>
    `;
...
function generateImage() {
    ...
    showToast('🎨 正在使用 ' + model + ' 生成图片...', 'info');

    // Simulate image generation with a delay
    setTimeout(() => {
        loadingDiv.style.display = 'none';

        // Generate a gradient-based placeholder image (simulated result)
        const canvas = document.createElement('canvas');
        canvas.width = 800;
        canvas.height = 450;
        const ctx = canvas.getContext('2d');

        // Create a nice gradient background based on the prompt content
        const grad = ctx.createLinearGradient(0, 0, canvas.width, canvas.height);
        grad.addColorStop(0, '#E8783A');
        grad.addColorStop(0.3, '#D4A234');
        grad.addColorStop(0.6, '#A8D5BA');
        grad.addColorStop(1, '#B8C5D6');
        ctx.fillStyle = grad;
        ...
        // Add a center callout (rounded rect)
        const rx = canvas.width/2 - 120, ry = canvas.height/2 - 30, rw = 240, rh = 60, rr = 12;
        ...
        ctx.fillText('✨ ' + model + ' 生成结果 ✨', canvas.width/2, canvas.height/2);

        // Add watermark-like label
        ctx.fillStyle = 'rgba(255,255,255,0.2)';
        ctx.font = '11px sans-serif';
        ctx.fillText(model + ' · Image Prompt Tool', canvas.width/2, canvas.height - 16);
